...
DATA_PATH = 'D:\\data\\bionlp' #os.path.join(os.path.expanduser('~'), 'biodata')
...
#			print owner
	...
	def build(self):
		return {'title':self.title, 'abs':'\n'.join(self.abs_list).strip(), 'mesh':self.mesh_headings, 'chem':self.chemicals}
...
def fetch_abs(pmid_list, saved_path=ABS_PATH):
	...
	if saved_path is not None:
		for id, abs in zip(pmid_list, abs_text):
			fs.write_file(os.path.join(ABS_PATH, str(id)+'.txt'), abs)
	return abs_text
	
	
def fetch_artcls(pmid_list, saved_path=DATA_PATH):
	feat_attr = {'abs':['abs', 'txt'], 'mesh':['mesh', 'mesh'], 'chem':['chem', 'chem']}
		...
		for id, artcls in zip(pmid_list, articles):
			for f, a in feat_attr.iteritems():
				if (hasattr(artcls[f], '__iter__')):
					...
					attr_text = artcls[f]
				fs.write_file(os.path.join(saved_path, a[0], str(id)+'.'+a[1]), attr_text)
	return articles